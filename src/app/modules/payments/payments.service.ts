...
const toClient = (payment: Awaited<ReturnType<typeof prisma.payment.findFirst>>) => {
...
const getPayments = async (query: Record<string, unknown>) => {
  ...
  const status =
    typeof query.status === 'string' && query.status ? toDbPaymentStatus(query.status) : undefined;
  const where = {
    ...(status && { status }),
    ...(search && {
      OR: [
