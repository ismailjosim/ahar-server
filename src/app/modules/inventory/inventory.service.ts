...
const getInventoryItems = async (query: Record<string, unknown>) => {
  ...
  if (lowStockOnly) {
    filteredData = filteredData.filter((item) => item && item.stock <= item.threshold);
  }
...
const getInventoryItemById = async (id: string) => {
  ...
  if (!item) throw new AppError(StatusCode.NOT_FOUND, 'Inventory item not found');
...
const createInventoryItem = async (payload: InventoryPayload) => {
  ...
  const item = await prisma.inventoryItem.create({
    data: {
      ...
      lastRestocked: payload.lastRestocked ? new Date(String(payload.lastRestocked)) : new Date(),
      audits: {
        create: {
          change: stock,
          previousStock: 0,
...
const updateInventoryItem = async (id: string, payload: InventoryPayload) => {
  ...
  if (!current) throw new AppError(StatusCode.NOT_FOUND, 'Inventory item not found');

  const nextStock = payload.stock === undefined ? current.stock : Number(payload.stock);
