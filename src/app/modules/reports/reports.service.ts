...
const exportCsv = async (type: string, from?: string, to?: string) => {
  ...
  if (type === 'inventory') {
    ...
    return [
      ['ID', 'Name', 'Category', 'SKU', 'Stock', 'Unit', 'Threshold'].join(','),
      ...items.map((item) =>
        [item.id, item.name, item.category, item.sku || '', item.stock, item.unit, item.threshold]
          .map(escapeCsv)
          .join(','),
      ),
    ].join('\n');
  ...
  if (type === 'reservations') {
    ...
    return [
      ['ID', 'Customer', 'Phone', 'Guests', 'Time', 'Table', 'Status'].join(','),
      ...reservations.map((reservation) =>
        [
          ...
          reservation.phone,
          reservation.guests,
          reservation.displayTime || reservation.reservationTime.toISOString(),
...
async function getDashboardStats() {
  ...
  const lowStockItems = inventoryItems.filter((i) => i.stock <= i.threshold).length;
...
async function getSummaryStats(from?: string, to?: string) {
  ...
  const [totalRevenue, totalOrders, avgOrderValue, topItems, ordersByStatus, paymentsByMethod] =
    await Promise.all([
