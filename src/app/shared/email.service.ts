...
export const EmailService = {
  ...
  async sendOrderStatusUpdate(order: {
    ...
    await sendMail(order.email, `আহার — Your order is ${order.status}`, html);
  },
  ...
  async sendReservationStatusUpdate(reservation: {
    ...
    await sendMail(reservation.email, `আহার — Reservation ${reservation.status}`, html);
  },
  ...
  async sendStaffInvite(invite: {
    ...
    await sendMail(invite.email, `আহার — You've been invited as ${invite.role}`, html);
  },
};
